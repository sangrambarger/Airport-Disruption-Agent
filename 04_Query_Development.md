...
Airport Disruption Signal Hunter Agent must not depend on fixed keyword lists. It must use LLM-based query development to create, expand, localize, and improve searches based on source type, geography, airport, language, event signal, and prior results.

## Query Development Agent Role

The Query Development Agent creates query packs for subagents.
...
## Inputs to Query Development

For each search cycle, consider:
...
- airport strike
- ground handling strike
- refuelling strike
- fuel workers strike
- security staff strike
- cargo handlers strike
- ATC strike
- customs strike
- walkout
- industrial action
...
- air cargo delays
- freight operations airport
- cargo flights cancelled
- customs cargo delay
- cargo warehouse closed
...
- check-in outage causing disruption
- airport systems failure
...
- security alert airport
- customs disruption airport
- cargo screening delay
- gas leak airport
- fire at airport operations
- explosion airport
...
- airport closed weather
- flights suspended due to weather
- fog disrupts airport operations
- snow closes airport
- storm disrupts airport
- airport diversions weather
...
At the end of each cycle, store:

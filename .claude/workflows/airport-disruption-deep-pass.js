export const meta = {
  name: 'airport-disruption-deep-pass',
  description: 'Recall-complete global multilingual sweep across all source buckets since the last successful deep pass - the not-miss-anything sweep, not the minutes-level one',
  phases: [
    { title: 'Discover', detail: 'all seven source buckets, global + multilingual queries' },
...
const PASS_TYPE = 'deep'
const FALLBACK_WINDOW = '10 hours'
...
const recencyInstruction = sinceTimestamp
  ? `Focus your search on signals first reported or materially updated since ${sinceTimestamp} (the last successful ${PASS_TYPE} pass), but - unlike the flash pass - don't treat that as a hard cutoff: a slow-burn story (an unresolved dispute, a recurring pattern) that predates it is still in scope if it's not already accurately reflected in the ledger. `
  : `No confirmed last-successful-pass timestamp was supplied - fall back to the last ${FALLBACK_WINDOW} as your primary window, while still following up on anything the ledger shows as still-unresolved regardless of age. `
...
const discoveries = await parallel(BUCKETS.map(b => () => agent(
  `You are the ${b.key} discovery role for the DEEP pass of a recall-first global airport-disruption signal ` +
  `hunter - the recall-complete sweep, not the fast narrow one. Read ${SKILL_DIR}/references/source-strategy.md, ` +
  `${SKILL_DIR}/references/query-development.md, and ${SKILL_DIR}/references/multilingual-map.md for tactics. ` +
  `Your bucket focus: ${b.focus}. ${regionScope}${excludeScope}${recencyInstruction}` +
  `Use WebSearch (and WebFetch to verify specific pages) to hunt globally, in English and relevant local ` +
  `languages, for credible signals of airport operational disruption: closures, runway/ATC/radar/cyber issues, ` +
  `strikes or labour action, cargo terminal or customs disruption, security incidents, weather impact, or ` +
  `reopening/recovery updates. Prefer enumerable official feeds over generic search where they exist for this ` +
  `bucket - hit them directly before falling back to open search. Search iteratively: after your first batch ` +
  `of queries, generate follow-up queries from new entities/keywords you've found, and keep going until two ` +
  `consecutive new queries surface nothing you haven't already seen - recall is this pass's whole purpose, so ` +
  `do not stop after 2-3 sources and do not stop at a fixed batch size. If a single source or page is ` +
  `unresponsive or a query yields nothing after two attempts, move on rather than retrying it further. ` +
  `Capture weak-but-credible signals; do not filter for relevance yourself, that happens in a later step. ` +
  `Return every candidate signal with full source attribution - a working source_url is required for each ` +
  `candidate - and fill impact_estimate from whatever the evidence actually states, leaving any field null ` +
...
log(`Discovery complete: ${merged.length} raw candidate signals across ${BUCKETS.length} deep buckets` + (pendingCandidates.length ? ` (including ${pendingCandidates.length} carried over from a prior failed cycle)` : ''))
...
try {
  classification = await agent(
    `You are the deduplication and classification stage for the DEEP pass of a recall-first airport-disruption ` +
